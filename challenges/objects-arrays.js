...
function Tyrannosaurus(dinosaur) {
    Dinosaur.call(this, dinosaur);
};
function Stegosaurus(dinosaur) {
    Dinosaur.call(this, dinosaur);
};
function Velociraptor(dinosaur) {
    Dinosaur.call(this, dinosaur);
};

// tyrannosaurus, carnivorous, 7000kg, 12m, Late Cretaceous
const tyrannosaurus = new Tyrannosaurus({name: 'tyrannosaurus',
    diet: 'carnivorous',
    weight: '700kg',
    length: '12m',
    period: 'Late Cretaceous'});
// stegosaurus, herbivorous, 2000kg, 9m, Late Jurassic
const stegosaurus = new Dinosaur({name: 'stegosaurus',
    diet: 'herbivorous',
    wight:'2000kg',
    length:'9m',
    period:'Late Jurassic'});
// velociraptor, carnivorous, 15kg, 1.8m, Late Cretaceous
const velociraptor = new Dinosaur({name: 'velociraptor' ,
    diet: 'carnivorous',
    weight: '15kg',
    length: '1.8m',
    period: 'Late Cretaceous'});
...
const universities =[];
graduates.forEach(person => universities.push(person.university));
...
const contactInfo = graduates.map(person =>{ return{ 'first_name': person.first_name, 'email': person.email}});
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = graduates.filter(person => person.university.slice(0,3) === 'Uni');
const justUni = [];
uni.forEach(university => justUni.push(university.university));
console.log(justUni);
...
const lowerCase = zooAnimals.map(animal => 'Name: ' + animal.animal_name.toUpperCase());
